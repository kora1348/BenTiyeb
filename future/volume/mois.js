async function fetchCryptoData(symbol) {
    try {
      const response = await fetch(
        `https://api.binance.com/api/v3/klines?symbol=${symbol}USDT&interval=1M&limit=8`
      );
      ...
      if (totalVariation >= 160) {
        totalCell.classList.add("positive");
      } else if (totalVariation <= -160) {
        totalCell.classList.add("negative");
      }
  
      if (totalVariation >= 160 || totalVariation <= -160) {
