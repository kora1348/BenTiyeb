async function fetchCryptoData(symbol) {
    try {
        // Dates en dur
        const startDate = new Date('2023-09-15T12:00:00Z');
        const endDate = new Date('2023-09-15T12:14:00Z');
        
        const startTime = startDate.getTime();
        const endTime = endDate.getTime();

        const response = await fetch(
            `https://api.binance.com/api/v3/klines?symbol=${symbol}USDT&interval=15m&startTime=${startTime}&endTime=${endTime}&limit=1`
        );
