...
async function updateCryptoData(symbol) {
  try {
    const response = await fetch(
      `https://api.binance.com/api/v3/klines?symbol=${symbol}USDT&interval=2h&limit=2`
    );
...
function checkAndNotify(symbol, totalVariation) {
  if (totalVariation >= 28) {
    ...
    if (!notificationState[symbol]) {
      ...
      createNotification(symbol, `2 heures: (Future)\nLa variation est supérieure ou égale à 28% (On Short) (${formattedTime})`);
    ...
    if (!notificationState[symbol]) {
      const currentTime = new Date();
      const formattedTime = `${currentTime.getHours()}:${currentTime.getMinutes()}:${currentTime.getSeconds()}`;
      createNotification(symbol, `2 heures: (Future)\nLa variation est inférieure ou égale à -28% (On Long) (${formattedTime})`);
      notificationState[symbol] = Date.now(); // Enregistrez le moment où la notification a été affichée
    }
  } else {
