// Variable globale pour stocker le total global
let globalTotalVariation = 0;
...
async function fetchCryptoData(symbol) {
    try {
      ...
      // Ajouter la variation totale de cette crypto au total global
      globalTotalVariation += totalVariation;
...
function updateGlobalTotal() {
    const globalTotalElement = document.getElementById("total-global-variation");
    const totalValue = globalTotalVariation.toFixed(2);
    
    globalTotalElement.textContent = `Total Global: ${totalValue}%`;
    globalTotalElement.style.fontWeight = "bold";
    globalTotalElement.style.margin = "20px 0";
    
    // Appliquer le style en fonction de la valeur
    if (globalTotalVariation > 0) {
        globalTotalElement.classList.add("positive");
        globalTotalElement.classList.remove("negative");
    } else if (globalTotalVariation < 0) {
        globalTotalElement.classList.add("negative");
        globalTotalElement.classList.remove("positive");
    } else {
        globalTotalElement.classList.remove("positive", "negative");
    }
}
