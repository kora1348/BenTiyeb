...
async function fetchCryptoData(symbol, startDate, endDate) {
  try {
    const response = await fetch(
       `https://api.binance.com/api/v3/klines?symbol=${symbol}USDT&interval=15m&startTime=${startDate}&endTime=${endDate}`
    );
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      variationCell.textContent = `${variationValue}%`; // Plus simple pour 15 minutes
...
function fetchDataForPeriod() {
  ...
  const [day, month, year, hours, minutes] = startDateStr.split(/[- :]/);
  const startTimestamp = new Date(year, month-1, day, hours, minutes).getTime();
  
  const [endDay, endMonth, endYear, endHours, endMinutes] = endDateStr.split(/[- :]/);
  const endTimestamp = new Date(endYear, endMonth-1, endDay, endHours, endMinutes).getTime();
