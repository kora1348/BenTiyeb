...
async function fetchCryptoData(symbol) {
    try {
        ...
        if (lastIntervalVariation >= 5.00 && lastIntervalVariation <= 5.99) {
            ...
            showPopup(`${symbol}: LONG signal détecté - 5 MINUTES (${lastIntervalVariation.toFixed(2)}%)`);
            ...
            cryptoNamesElement.appendChild(pElement);
            showPopup(`${symbol}: SHORT signal détecté - 5 MINUTES (${lastIntervalVariation.toFixed(2)}%)`);
        }
    } catch (error) {
