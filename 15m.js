async function fetchCryptoData(symbol) {
    try {
        const response = await fetch(
            `https://api.binance.com/api/v3/klines?symbol=${symbol}USDT&interval=15m&limit=10`
        );
        ...
        const averageVariation = (totalVariation / data.length).toFixed(2);
  
        // Ajouter la cellule pour afficher la moyenne des intervalles
