...
function checkAndDisplayLong(symbol) {
    const percentage30m = cryptoResults5m[symbol];
    const percentage1h = cryptoResults15m[symbol];

    // Vérifie si les deux pourcentages sont dans les plages définies
    if (percentage30m >= 60 && percentage30m <= 69 && percentage1h >= 80 && percentage1h <= 89) {
    ...
    } else if (percentage30m >= -69 && percentage30m <= -60 && percentage1h >= -89 && percentage1h <= -80) {
        const cryptoNamesDiv = document.getElementById('cryptoNames');
...
async function fetchCryptoData5Min(symbol) {
    try {
        const response = await fetch(`https://api.binance.com/api/v3/klines?symbol=${symbol}USDT&interval=30m&limit=1`);
        ...
        const cryptoRow = document.getElementById(`${symbol}_30m`);
        ...
        console.error(`Erreur lors de la récupération des données pour ${symbol} (30m):`, error);
    }
}
