async function fetchCryptoData(symbol) {
    try {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            // Détecter une bougie rouge avec une mèche vers le haut
            if (closePrice < openPrice && highPrice > openPrice) {
                console.log(`Bougie rouge avec mèche vers le haut détectée pour ${symbol} à l'intervalle ${i}`);
                variationCell.classList.add("wick-up-red");
                variationCell.textContent += " (Bougie rouge avec mèche vers le haut)";

                // Afficher le symbole de la crypto dans la div avec l'ID cryptoNames avec la classe "positive"
                const cryptoNamesDiv = document.getElementById('cryptoNames');
                const cryptoSymbolElement = document.createElement('div');
                cryptoSymbolElement.textContent = symbol;
                cryptoSymbolElement.classList.add("positive"); // Ajout de la classe "positive"
                cryptoNamesDiv.appendChild(cryptoSymbolElement);
            }
        }
