async function fetchCryptoData(symbol) {
  try {
    const response = await fetch(
      `https://api.binance.com/api/v3/klines?symbol=${symbol}USDT&interval=3m&limit=16`
    );
    ...
    if (isShortSignal) {
      ...
      showNotification(`${symbol}: Signal SHORT - 3m`);
    } else {
    ...
    if (isLongSignal) {
      ...
      showNotification(`${symbol}: Signal LONG - 3m`);
    } else {
