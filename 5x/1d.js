  ...
  async function fetchCryptoData(symbol) {
    try {
        ...
        if (totalVariation >= 49.00) {
            ...
            showPopup(`${symbol}: SHORT signal détecté - 1 DAY(${totalVariation.toFixed(2)}%)`);
        }  else if (totalVariation  <= -49.00) {
            ...
            cryptoNamesElement.appendChild(pElement);
            showPopup(`${symbol}: LONG signal détecté - 1 DAY(${totalVariation.toFixed(2)}%)`);
        }
    } catch (error) {
