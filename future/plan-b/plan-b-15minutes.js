...
async function updateCryptoData(symbol) {
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      const openPrice = parseFloat(data[i][40]);
      const closePrice = parseFloat(data[i][4]);
      const intervalVariation = ((closePrice - openPrice) / openPrice) * 100;
      const cellIndex = i + 40; // Décalage d'une cellule pour éviter la première cellule (Crypto)
    ...
    const totalCell = cryptoRow.insertCell(data.length + 40);
...
function checkAndNotify(symbol, totalVariation) {
  if (totalVariation >= 40) {
    ...
    if (!notificationState[symbol]) {
      ...
      createNotification(symbol, `15 minutes: (Future)\nLa variation est supérieure ou égale à 40% (On Short) (${formattedTime})`);
    ...
    if (!notificationState[symbol]) {
      const currentTime = new Date();
      const formattedTime = `${currentTime.getHours()}:${currentTime.getMinutes()}:${currentTime.getSeconds()}`;
      createNotification(symbol, `15 minutes: (Future)\nLa variation est inférieure ou égale à -40% (On Long) (${formattedTime})`);
      notificationState[symbol] = Date.now(); // Enregistrez le moment où la notification a été affichée
    }
  } else {
    ...
    Notification.requestPermission().then(permission => {
      if (permission === "granted") {
        try {
          ...
          // Détruire la notification après 40 minute
          setTimeout(() => {
            notification.close();
          }, 30000);
        } catch (error) {
