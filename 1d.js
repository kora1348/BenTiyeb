async function fetchCryptoData(symbol, years = [2022, 2023, 2024]) {
    try {
        ...
        for (const year of years) {
            const now = new Date();
            const startDate = new Date(year, now.getMonth(), now.getDate()).getTime();
            const endDate = new Date(startDate + 24 * 60 * 60 * 1000).getTime();

            const response = await fetch(
                `https://api.binance.com/api/v3/klines?symbol=${symbol}USDT&interval=1d&limit=1&startTime=${startDate}&endTime=${endDate}`
            );
            ...
            for (let i = 0; i < data.length; i++) {
                ...
                variationCell.textContent = `${startDate.toLocaleDateString("fr-FR", optionsDate)} : ${variationValue}%`;
