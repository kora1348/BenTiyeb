async function fetchCryptoData(symbol, years = [2022, 2023, 2024]) {
    try {
        ...
        for (const year of years) {
            const now = new Date();
            const startDate = new Date(year, now.getMonth(), now.getDate(), now.getHours(), now.getMinutes() - now.getMinutes() % 15).getTime();
            const endDate = new Date(startDate + 4 * 60 * 60 * 1000).getTime(); // 4 hours in milliseconds

            const response = await fetch(
                `https://api.binance.com/api/v3/klines?symbol=${symbol}USDT&interval=4h&limit=1&startTime=${startDate}&endTime=${endDate}`
            );
