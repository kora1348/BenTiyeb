...
async function updateCryptoData(symbol) {
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      const openPrice = parseFloat(data[i][50]);
      const closePrice = parseFloat(data[i][4]);
      const intervalVariation = ((closePrice - openPrice) / openPrice) * 100;
      const cellIndex = i + 50; // Décalage d'une cellule pour éviter la première cellule (Crypto)
    ...
    const totalCell = cryptoRow.insertCell(data.length + 50);
    const totalValue = totalVariation.toFixed(2);
    totalCell.textContent = `${totalValue}%`;

    if (totalVariation >= 50) {
      // Ajouter la classe CSS appropriée à totalCell
      totalCell.classList.add("positive");
    } else if (totalVariation <= -50) {
      // Ajouter la classe CSS appropriée à totalCell
...
function checkAndNotify(symbol, totalVariation) {
  if (totalVariation >= 50) {
    ...
    if (!notificationState[symbol]) {
      ...
      createNotification(symbol, `5 minutes: (Future)\nLa variation est supérieure ou égale à 50% (On Short) (${formattedTime})`);
    ...
    if (!notificationState[symbol]) {
      const currentTime = new Date();
      const formattedTime = `${currentTime.getHours()}:${currentTime.getMinutes()}:${currentTime.getSeconds()}`;
      createNotification(symbol, `5 minutes: (Future)\nLa variation est inférieure ou égale à -50% (On Long) (${formattedTime})`);
      notificationState[symbol] = Date.now(); // Enregistrez le moment où la notification a été affichée
    }
  } else {
    ...
    Notification.requestPermission().then(permission => {
      if (permission === "granted") {
        try {
          ...
          // Détruire la notification après 50 minute
          setTimeout(() => {
            notification.close();
          }, 30000);
        } catch (error) {
