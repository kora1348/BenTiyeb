...
async function fetchCryptoData(symbol) {
    try {
        const response = await fetch(`https://api.binance.com/api/v3/klines?symbol=${symbol}USDT&interval=1d&limit=4`);
        const data = await response.json();

        // Calcul du total des taux de variation sur 8 jours
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            const formattedDate = `${(dateValue.getDate() < 10 ? '0' : '') + dateValue.getDate()}/${(dateValue.getMonth() + 1 < 10 ? '0' : '') + (dateValue.getMonth() + 1)}/${dateValue.getFullYear().toString().slice(-2)}`;
            variationCell.textContent = `${formattedDate}: ${variationValue}%`;

            // Ne pas ajouter de classe de couleur aux cellules individuelles
            totalVariation += dailyVariation; // Ajouter la variation quotidienne au total
        }
