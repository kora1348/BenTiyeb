// Liste des cryptomonnaies à analyser (paires USDT disponibles sur Binance)
const cryptos = [
  "BTC", "ETH", "BNB", "SOL", "XRP", 
...
let interval = "1w"; // 1w pour weekly, 1d pour daily
...
// 1. Fonction principale - Lance tout le processus
async function main() {
  showLoading(); // Affiche un message de chargement
...
function showError(message) {
  ...
  if (tableBody) {
    tableBody.innerHTML = 
      `<tr><td colspan="10" style="color: red; text-align: center;">${message}</td></tr>`;
  }
...
function updateTable(filter = "ALL") {
  ...
  if (title) {
    title.textContent = currentDate
      ? `Trading Weekly (${new Date(currentDate).toLocaleDateString('fr-FR')})`
      : "Trading Weekly - Live";
  }
...
// 7. Change la timeframe (1w ou 1d)
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  // Actualise les données toutes les minutes
