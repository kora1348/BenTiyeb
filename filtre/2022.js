async function fetchCryptoDataFor2022(symbol, cellIndex) {
    // Récupérer la date et l'heure actuelles
    const currentDate = new Date();

    // Fixer l'année cible à 2022
    const targetYear = 2022;

    // Ajuster l'année de la date cible
    currentDate.setFullYear(targetYear);
    
    // Convertir la date cible en timestamp (date actuelle mais avec l'année 2022)
    ...
    try {
        ...
        if (data.length === 0) {
            console.log(`Aucune donnée trouvée pour ${symbol} en 2022`);
            return;
        }
        ...
        console.error(
            `Erreur lors de la récupération des données pour ${symbol} en 2022:`,
            error
        );
    }
}

// Appel de la fonction uniquement pour l'année 2022
fetchCryptoDataFor2022("BTC", 1); // Cellule pour l'année 2022
