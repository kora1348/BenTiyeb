...
async function fetchCryptoData(symbol) {
    try {
        const response = await fetch(
            `https://api.binance.com/api/v3/klines?symbol=${symbol}USDT&interval=3m&limit=6`
        );
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            // Stocker la variation du dernier intervalle (6ème)
            if (i === data.length - 1) {
                lastIntervalVariation = variation;
            }
        }

        const totalCell = cryptoRow.insertCell(-1);
        totalCell.textContent = `${lastIntervalVariation.toFixed(2)}%`;
        ...
        if ( (lastIntervalVariation >= 3.00 && lastIntervalVariation <= 3.99) || (lastIntervalVariation >= 5.00 && lastIntervalVariation <= 5.99) ) {
            totalCell.classList.add("positive");
            const pElement = document.createElement("p");
            pElement.id = `${symbol}_status`;
            pElement.classList.add("positive");
            pElement.textContent = `${symbol}: LONG, ${lastIntervalVariation.toFixed(2)}%`;
            cryptoNamesElement.appendChild(pElement);
            showPopup(`${symbol}: LONG signal détecté - 3 MINUTES (${lastIntervalVariation.toFixed(2)}%)`);
        } else if ( (lastIntervalVariation >= -3.99 && lastIntervalVariation <= -3.00) || (lastIntervalVariation >= -5.99 && lastIntervalVariation <= -5.00) ) {
            totalCell.classList.add("negative");
            const pElement = document.createElement("p");
            pElement.id = `${symbol}_status`;
            pElement.classList.add("negative");
            pElement.textContent = `${symbol}: SHORT, ${lastIntervalVariation.toFixed(2)}%`;
            cryptoNamesElement.appendChild(pElement);
            showPopup(`${symbol}: SHORT signal détecté - 3 MINUTES (${lastIntervalVariation.toFixed(2)}%)`);
        }
    } catch (error) {
...
function startAutoRefresh() {
    const cryptoSymbols = ["1INCH", "AAVE", "ACE", "ACH", "ADA", "AEVO", "AGLD", "AI", "ALGO", "ALICE", "ALPACA", "ALPHA",
        ...
        "XEM", "XLM", "XRP", "XTZ", "XVG", "XVS", "YFI", "YGG", "ZEC", "ZIL", "ZK", "ZRO", "ZRX"]; //outez d'autres symboles crypto si nécessaire
