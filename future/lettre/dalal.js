async function fetchCryptoData(symbol) {
  try {
    ...
    if (totalVariation >= 0.79 && totalVariation <= 0.90) {
      totalCell.classList.add("negative");
    } else if (totalVariation <= -0.79 && totalVariation >= -0.90) {
      totalCell.classList.add("positive");
    }

    if (totalVariation == 0.76) {
      totalCell.classList.add("positive");
    } else if (totalVariation == -0.76) {
      totalCell.classList.add("negative");
    }

    if ( (totalVariation >= 0.79 && totalVariation <= 0.90) ||  (totalVariation <= -0.79 && totalVariation >= -0.90)) {
    ...
    if ( (totalVariation == 0.76) ||  (totalVariation == -0.76)) {
      // Ajouter le nom de la crypto en dehors du tableau
      const cryptoNameDiv = document.getElementById("cryptoNames");
      const cryptoName = document.createElement("p");
      cryptoName.textContent = `${symbol} : ${totalValue}%`;
