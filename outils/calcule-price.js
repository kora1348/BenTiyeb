...
function calculateDay() {
  ...
  // Calcul de +5%
  const resultPlus5 = number + (number * 5) / 100;
  output += `<div class="negative">${resultPlus5.toFixed(10)}</div>`;

  // Ajout de la valeur de l'input avec la classe "blue"
  output += `<div class="blue">${number.toFixed(10)}*</div>`;

  // Calcul de -5%
  const resultMinus5 = number - (number * 5) / 100;
  output += `<div class="positive">${resultMinus5.toFixed(10)}</div>`;
