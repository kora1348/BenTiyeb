async function fetchCryptoData(symbol, years = [2022, 2023, 2024]) {
    try {
        ...
        for (const year of years) {
            ...
            const endDate = new Date(startDate + 1 * 60 * 60 * 1000).getTime(); // 4 hours in milliseconds

            const response = await fetch(
                `https://api.binance.com/api/v3/klines?symbol=${symbol}USDT&interval=1h&limit=1&startTime=${startDate}&endTime=${endDate}`
            );
