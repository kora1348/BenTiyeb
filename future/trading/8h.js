async function fetchCryptoData(symbol) {
  try {
    const response = await fetch(
      `https://api.binance.com/api/v3/klines?symbol=${symbol}USDT&interval=8h&limit=16`
    );
    ...
    if (isShortSignal) {
      ...
      showNotification(`${symbol}: Signal SHORT - 8h`);
    } else {
    ...
    if (isLongSignal) {
      ...
      showNotification(`${symbol}: Signal LONG - 8h`);
    } else {
